...
export default function Auth({ forceMode }) {
  ...
  const [mode, setMode] = useState(() => {
    ...
    if (isPasswordRecovery) return 'update_password';
    // First-time visitors on this device see Sign Up; returning devices see Sign In.
    if (typeof window !== 'undefined') {
      try {
        if (!localStorage.getItem('sscbs_has_visited')) return 'signup';
      } catch (e) {}
    }
    return 'signin';
  });
          ...
          <div className="auth-card-outer">
            <div className="auth-card-inner">
              ...
              {mode !== 'forgot' && mode !== 'update_password' && (
                <>
                  <button type="button" className="google-signin-button google-primary" onClick={handleGoogleSignIn} disabled={loading}>
                  ...
                  <div className="form-input-group">
                    ...
                    {mode === 'signup' && (
                      <div className="field-hint">
                        🔒 Create a <strong>new password just for Campus OS</strong> — this is <strong>not</strong> your college email password.
                      </div>
                    )}
                    {mode === 'signin' && (
                      <div className="field-hint">
                        Use the password you created for Campus OS (not your college email password).
                      </div>
                    )}
                  </div>
                )}
